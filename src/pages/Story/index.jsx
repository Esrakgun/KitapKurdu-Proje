...
function Story() {
  ...
  // useEffect(() => {
  //   axios
  //     .get("http://localhost:5001/books?type=") // API'den veri alıyoruz
  //     .then((response) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5001/books?type=story") // API'den veri alıyoruz
      .then((response) => {
       setBooks(response.data); // Hikaye kategorisindeki kitapları set et
      })
      .catch((error) => console.error("Error fetching books:", error));
  }, []);
